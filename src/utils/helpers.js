...
// Cache for file type regex dictionary to avoid repeated object and RegExp creation on every call
const fileTypeRegex = {
	babel: /\.babelrc$/i,
	jsmap: /\.js\.map$/i,
	yarn: /^yarn\.lock$/i,
...
function getFileType(filename) {
	// Use highly-optimized, allocation-free loop for maximum performance and garbage collection avoidance
	for (const type in fileTypeRegex) {
		if (fileTypeRegex[type].test(filename)) {
			return type;
		}
	}
