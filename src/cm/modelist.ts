...
export function getModeForPath(path: string): Mode {
	...
	// Avoid redundant O(M log M) sorting and O(M) specificity calculation on every call by utilizing cachedSortedModes
...
export class Mode {
	...
	specificityScore: number; // Pre-calculated specificity score for fast sorting
